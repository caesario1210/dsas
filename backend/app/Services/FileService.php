...
class FileService
{
    ...
    public function validateAndParseFile(UploadedFile $file): array
    {
        ...
        $data = $this->parseFileByExtension($fullPath, $extension);
    ...
    protected function storeTemporarily(UploadedFile $file, string $extension): string
    {
        ...
        $filename = 'upload_' . time() . '_' . Str::random(10) . '.' . $extension;
    ...
    protected function parseFileByExtension(string $fullPath, string $extension): array
    {
        if ($extension === 'csv') {
